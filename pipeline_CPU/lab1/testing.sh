...
declare -a inputfilelist=("add.x" "addi.x" "adds.x" "addis.x" "cbnz.x" "cbz.x" "and.x" "ands.x" 
	"eor.x" "orr.x" "ldur.x" "ldurb.x" "ldurh.x" "lsl.x" "lsli.x" "lsri.x" "movz.x" "stur.x" "sturb.x" 
	"sturh.x" "sub.x" "subi.x" "subs.x" "subsi.x" "mul.x" "cmp.x" "br.x" "b.x" "beq.x" "bne.x" 
	"bgt.x" "blt.x" "bge.x" "ble.x" "countdigits.x" "fibonacci.x" "sturldur.x" "verycomplicatedloadandstur.x"
	"supercomplicatedbranching.x" "verycomplicatedarithmetic.x" "step1-a-inputs.x")
...
echo "Correct tests: $CORRECT_TESTS/$NUM_TESTS"  >> "lab1-tests.txt"
