...
/* 
    We used one late day on this lab.
    Revision 470 on November 17 @ 10:40 pm CST
*/
...
/* initialize branch prediction structures here */
...
/* for branch prediction flushing */
int FLAG_WBP = 0;
int helper_WBP = 0;
...
void pipe_stage_wb() {
	...
	if (MEMWB.opcode) {
        ...
        if(prev_inst == 0x6b0 && MEMWB.opcode == 0x6b0){
            stat_inst_retire -=1;
            if(dirtyop){
                stat_inst_retire += 1;
                dirtyop = 0;
        ...
        if(prev_inst == 0x54 && MEMWB.opcode == 0x54){
            stat_inst_retire -=1;
            if(dirtyop){
                stat_inst_retire += 1;
                dirtyop = 0;
        ...
        if(prev_inst == 0xB5 && MEMWB.opcode == 0xB5){
            stat_inst_retire -=1;
        }
    ...
    prev_pc = MEMWB.PC;
    CURRENT_STATE.REGS[31] = 0; 
    //printf("Instruction count retired: %d\n", stat_inst_retire);
	...
	if (!FLAG_L) { 
        ...
        switch(IDEX.opcode) {
            ...
            case 0x6B0: { // BR
                ...
                /* check if necessary to flush */
                //bp_checker(IDEX.predicted_PC, actual_PC, &match, &taken);
            ...
            case 0x5: { // B
                ...
                if(IDEX.miss == 1){
                    ...
                    helper_WBP = 1; 
                    CURRENT_STATE.PC = actual_PC; 
                } /*else if(CURRENT_BP.g_share.PHT[CURRENT_BP.g_share.GHR ^ gshare_pc(IDEX.fetch_PC)] < 2){
                    FLAG_WBP = 1;
                    helper_WBP = 1; 
                    CURRENT_STATE.PC = actual_PC;  
                }*/ else if(actual_PC != IDEX.predicted_PC){
            ...
            case 0xB4: { // CBZ
                ...
                if(IDEX.miss == 1 && taken == 1){
                    ...
                    helper_WBP = 1; 
                    CURRENT_STATE.PC = actual_PC; 
                } /*else if(taken == 1 && CURRENT_BP.g_share.PHT[CURRENT_BP.g_share.GHR ^ gshare_pc(IDEX.fetch_PC)] < 2){
                    FLAG_WBP = 1;
                    helper_WBP = 1; 
                    CURRENT_STATE.PC = actual_PC;  
                } else if(taken == 0 && CURRENT_BP.g_share.PHT[CURRENT_BP.g_share.GHR ^ gshare_pc(IDEX.fetch_PC)] > 1){
                    FLAG_WBP = 1;
                    helper_WBP = 1; 
                    CURRENT_STATE.PC = actual_PC;  
                }*/ else if(actual_PC != IDEX.predicted_PC){
            ...
            case 0x54: { // b.cond
                ...
                if(IDEX.miss == 1 && taken == 1){
                    ...
                    FLAG_WBP = 1;
                    helper_WBP = 1; 
                    printf("wrong predict 1");
                    CURRENT_STATE.PC = actual_PC;  
                } else if(taken == 0 && CURRENT_BP.g_share.PHT[CURRENT_BP.g_share.GHR ^ gshare_pc(IDEX.fetch_PC)] > 1){
                    FLAG_WBP = 1;
                    helper_WBP = 1;
                    printf("wrong predict 0"); 
                    CURRENT_STATE.PC = actual_PC;  
                }*/ else if(actual_PC != IDEX.predicted_PC){
                    FLAG_WBP = 1;
                    helper_WBP = 1; 
...
void pipe_stage_fetch(uint64_t address) {
    //printf("Current GHR: %x\n", CURRENT_BP.g_share.GHR);
    //for (int i = 0; i < 256; i++) {
	...
	if (!FLAG_H) {
        uint64_t next_PC; 
        int miss; 
        if (!FLAG_L && !FLAG_WBP) { // no load-use dependency 
            ...
            IFID.tbp = CURRENT_BP.g_share.PHT[CURRENT_BP.g_share.GHR ^ gshare_pc(IDEX.fetch_PC)];
            //printf("Fetch PC: %lx\n", IFID.fetch_PC);
            //printf("Next PC: %lx\n", next_PC);
            IFID.miss = miss; 
            //IFID.PC_btb = PC_btb;
            //IFID.xor_predictor = xor_predictor;
